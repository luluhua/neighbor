    ...
    <section class="content-header">
        <h1>
            <small>
                ...
                <a href="/system/sysCity/streetlist/1?pid=#{(area.id)!}"> [${(province.name)}][${(city.name)}][${(area.name)}][${(street.name)}]</a> >
                居委会列表
            </small>
        </h1>
    </section>
    ...
    <section class="content">
        ...
        <tr>
            ...
            <#include "../../common/paginateBar.ftl" />
	  		  <@paginate pageData=pageData actionUrl="/system/sysCity/roadlist/" urlParas="?search=${search!}"  />
                        </div>
                        <div class="col-md-6 pull-left">
            <#include "../../common/paginate.ftl" />
            <@paginate currentPage=pageData.getCurrent() totalPage=pageData.getPages() actionUrl="/system/sysCity/roadlist/" urlParas="?search=${search!}&pageSize=${pageSize!}"  />
                        </div>
                    </div>
                </div><!-- /.box -->
            </div>
