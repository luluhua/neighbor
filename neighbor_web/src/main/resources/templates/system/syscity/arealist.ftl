...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>
                ...
                <a href="/system/sysCity/citylist/1?pid=#{(province.id)!}"> [${(province.name)}] [${(city.name)}]</a> >
                区/县列表
            </small>
        </h1>
    </section>
    ...
    <section class="content">
        ...
        <tr>
            <td><a href="/system/sysCity/streetlist/1?pid=#{(entity.id)!}">${(entity.name)!'--'}</a></td>
            <td>${(entity.code)!'--'}</td>
            ...
            <#include "../../common/paginateBar.ftl" />
	  		  <@paginate pageData=pageData actionUrl="/system/sysCity/arealist/" urlParas="?search=${search!}"  />
                        </div>
                        <div class="col-md-6 pull-left">
            <#include "../../common/paginate.ftl" />
            <@paginate currentPage=pageData.getCurrent() totalPage=pageData.getPages() actionUrl="/system/sysCity/arealist/" urlParas="?search=${search!}&pageSize=${pageSize!}"  />
                        </div>
                    </div>
                </div><!-- /.box -->
            </div>
