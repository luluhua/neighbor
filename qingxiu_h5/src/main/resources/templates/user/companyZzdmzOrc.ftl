            ...
            <div class="mui-scroll" id="boodylie">
                <div class="box">
                    <div class="top-background">
                        <b>企业组织代码识别</b>
                        <span>请上传企业组织代码照片，保证字体清晰无遮挡</span>
                    ...
                    <div class="img_ingse" id="img_ingse_Id">
                        ...
                        <div class="img_living">
                            <img src="${ctx}/user/images/sydw.jpg"/>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function doCompanyZzdmzOrc(imgurl) {
        if(_gToken==''){
            mui.toast("未登录", {duration: '3500ms', type: 'div'});
            return;
        }
        ...
        $.ajax({
            async: true,
            type: "POST",
            url: "${ctx}/qingxiuh5/appUser/doCompanyZzdmzOrc",
            data: {
                "imgUrl": _imgurl
            },
            ...
            success: function (json) {
                layer.load(1,{time:1});
                if(json.code==200){
                    $("#code").html(json.data.zch);
                    $("#name").html(json.data.fddb);
                    $("#address").html(json.data.zczb);
                    $("#yxq").html(json.data.gsmc);
                    $("#companyType").html(json.data.companyType);
                    $("#issuingUnit").html(json.data.issuingUnit);
                    $("#registrationNo").html(json.data.registrationNo);
