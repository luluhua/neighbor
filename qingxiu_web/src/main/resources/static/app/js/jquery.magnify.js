...
(function (factory) {
    ...
    function getImgSrcid(el) {
        // Get data-src as image src at first
        var src = $(el).attr('data-id');
        return src;
    }

    function getImgType(el) {
    ...
    var $W = $(window),
        ...
        DEFAULTS = {
            ...
            icons: {
                ...
                rotateSign: '<svg viewBox="0 0 877.7142857142857 1024" class="svg-inline-icon">\
                               <img class="biaojieae" xlink:href="../../../app/images/biaoji.png" src="../../../app/images/biaoji.png" style="width: 20px;height: 20px;margin-top: -30px;" />\
                                </svg>'
            },
...
// jQuery element of calling plugin
    ...
    Magnify.prototype = {
        ...
        init: function (el, opts) {
            ...
            var imgid = getImgSrcid(el);

            var imgtype = getImgType(el);


            // Get image group
            this.groupName = null;
            grose = $(el).attr('data-group');
            ...
            if (currentGroupName !== undefined) {
                ...
                this.getImgGroup(groupList, imgSrc, imgid, imgtype);
            } else {
                this.getImgGroup(jqEl.not('[data-group]'), imgSrc, imgid, imgtype);
            }

            this.open();

            this.loadImg(imgSrc, imgid, imgtype);
        ...
        render: function () {
            ...
            if (grose == 1) {
                btnsTpl = {
                    ...
                    '</button>',
                    rotateSign:
                    '<button class="magnify-button magnify-button-rotate-Sign" title="' +
                    this.options.i18n.rotateSign +
                    '">' +
                    this.options.icons.rotateSign +
                    '</button>'
                };
            } else {
                btnsTpl = {
        ...
        build: function () {
            ...
            this.$rotateSign = $magnify.find('.magnify-button-rotate-Sign');
        ...
        loadImg: function (imgSrc, imgid, imgtype, fn, err) {
            var self = this;
            ...
            if (imgtype == 1) {
                $(".biaojieae").attr('src', '../../../app/images/biaoji2.png')
                $(".magnify-button-rotate-Sign").attr('title', "取消标记");
            } else {
                $(".biaojieae").attr('src', '../../../app/images/biaoji.png')
                $(".magnify-button-rotate-Sign").attr('title', "标记为不合格材料");
            }
            if (isIE8()) {
                this.$stage.html(
                    '<img class="magnify-image" id="magnify-image" title="' + imgid + '" ty="' + imgtype + '" src="' +
                    imgSrc +
                    '" alt="" />'
                );
            } else {
                this.$image.attr('src', imgSrc);
                this.$image.attr('title', imgid);
                this.$image.attr('ty', imgtype);
            }
            ...
            if (this.options.title) {
                this.setImgTitle(imgSrc, imgid, imgtype);
            }
        },
        getImgGroup: function (list, imgSrc, imgid, imgtype) {
            var self = this;
            ...
            $(list).each(function (index, item) {
                ...
                var imgids = getImgSrcid(this);
                var imgtype = getImgType(this);
                self.groupData.push({
                    src: src,
                    title: imgids,
                    ty: imgtype,
        ...
        jumpTo: function (index) {
            ...
            this.loadImg(
                ...
                this.groupData[index].title,
                this.groupData[index].ty,
        ...
        Sign: function () {
            var imgid = $(".magnify-image").attr('title');  //获取图片标识
            var type = $(".magnify-image").attr('ty');  //获取图片状态
            $(".magnify-button-rotate-Sign").attr("disabled", true);  //点击后禁止按钮再点击
            $.post('/appc/tGovRecord/materialsSign/' + imgid + "?type=" + type, function (response) {
                if (response.code == 200) {
                    layer.msg('操作成功', {
                        ...
                        time: 500,
                            ...
                            if (type == 0) {
                                $("#" + imgid + "").css({'opacity': '0.3'});   //添加图片遮罩层
                                $("#" + imgid + "s").css({'display': 'block'});  //显示提示文字
                                $(".biaojieae").attr('src', '../../../app/images/biaoji2.png'); // 更换按钮icon
                                $("#" + imgid + "").attr('data-type', '1'); // 更新图片状态
                                $(".magnify-image").attr('ty', '1'); // 更新图片状态
                                $(".magnify-button-rotate-Sign").attr('title', "取消标记"); // 更新按钮提示
                                $(".magnify-button-rotate-Sign").attr("disabled", false); // 放开按钮点击事件
                            } else {
                                $("#" + imgid + "").css({'opacity': '1'});
                                $("#" + imgid + "s").css({'display': 'none'});
                                $(".biaojieae").attr('src', '../../../app/images/biaoji.png')
                                $("#" + imgid + "").attr('data-type', '0');
                                $(".magnify-image").attr('ty', '0');
                                $(".magnify-button-rotate-Sign").attr('title', "标记为不合格材料");
                                $(".magnify-button-rotate-Sign").attr("disabled", false);
                            }
                        }
                    })
