...
<form class="form-horizontal layui-form layui-form-pane" method="post"
    <#if (entity.id)??>
      action="/system/tTag/doEdit"
    <#else>
      action="/system/tTag/doAdd"
    </#if>
>
    <#if (entity.id)??><input type="hidden" name="id" value="${(entity.id)}"/></#if>
    <input type="hidden" id="navigationId" name="navigationId"
           value="${(navigationId)!}" class="form-control" placeholder="请输入导航Id"
           lay-verify="required"/>
    <input type="hidden" id="pid" name="pid"
           value="${(pid)!}" class="form-control" placeholder="pid"
           lay-verify="required"/>
    <div class="box-body">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input type="text" id="code" name="code"
                       value="${(entity.code)!}" class="form-control" placeholder="请输入code"
                       lay-verify="required"/>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input type="text" id="skipUrl" name="skipUrl"
                       value="${(entity.skipUrl)!}" class="form-control" placeholder="请输入跳转URL"
                />
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-10">
            <#--<input type="number" id="skipType" name="skipType"-->
            <#--value="${(entity.skipType)!}" class="form-control" placeholder="请输入跳转类型 0直接打开 1新页面打开 2 弹出层"-->
            <#--lay-verify="required"/>-->
         <@dictTag method="getDictListByType" type="skip_Type" >
             <#list getDictListByType as m>
                     <input type="radio" name="skipType" value="${(m.value)!}" lay-filter="skipType"
                            title="${(m.label)!}"
                             <#if "${(m.value)!}"=="${(entity.skipType)!}">checked</#if>>
             </#list>
         </@dictTag>
