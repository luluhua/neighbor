                    ...
                    <div class="box-body">
                        ...
                        <div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input id="batchType"  name="batchType" type="checkbox" class="minimal" value="1" lay-ignore>
                                    批量添加下级行政区划
                                </div>
                            </div>
                        </div>
...
<@footer>
    ...
    <script type="application/javascript">
        ...
        var TableInit = function () {
            ...
            oTableInit.Init = function () {
                $('#tableId').bootstrapTable({
                    url: '/appc/tGovProjectCityOrgAdress/projectJson',         //请求后台的URL（*）
                    method: 'get',                      //请求方式（*）
        ...
        function onSubmit() {
            onSelectProject();
            var onBatchAdd = "/appc/tGovProjectCityOrgAdress/doAddBatch";
            var batchType = 0;
            if ($("input[name='batchType']").prop("checked")) {
                            batchType=1;
            }
            var serviceType = $("#serviceTypeId").val();
            var projectIds = $("#projectIds").val();
            var cityId = $("#xzqhId").val();
            var streeId = $("#xzqhId2").val();
            if(serviceType==null||serviceType==""||
                    projectIds==null||projectIds==""||
                cityId==null||cityId==""){
                return;
            }
            // var communityId = $("#xzqhId3").val();
            $.post(onBatchAdd, {projectIds: projectIds, serviceType: serviceType,
                        batchType: batchType, cityId: cityId,
                        streeId:streeId},
                    function (json) {
                        if (json.code == 200) {
                            $.alert({
